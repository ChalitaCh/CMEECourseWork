...
for (i in select_data){
  ...
  tempdf$ID <- c(i)
  tempdf$Temp <- unique(data_raw$Temp[data_raw$ID == i])
...
S1 <- ggplot(data_raw[data_raw$ID %in% select_data,], aes(x = Time, y = logPopBio)) +
  geom_point() +
  ...
  labs(x ="Time (Hours)",
       y = "log(OD_595)",
       color = "Models",
       title = "Model fitness of Pseudomonas fluorescens in different temperatures") +
  theme_bw() +
  theme(aspect.ratio = 1)

ggsave("../results/S1.pdf", plot = S1,
       width = 29.7, height = 21.0, units = "cm")
